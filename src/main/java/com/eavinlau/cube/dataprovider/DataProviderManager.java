...
public class DataProviderManager implements ApplicationContextAware {
    ...
    static {
        Set<Class<?>> classSet = new Reflections("com.eavinlau.cube").getTypesAnnotatedWith(ProviderName.class);
        for (Class c : classSet) {
            if (!c.isAssignableFrom(DataProvider.class)) {
                providers.put(((ProviderName) c.getAnnotation(ProviderName.class)).name(), c);
            } else {
                System.out.println("自定义DataProvider需要继承com.eavinlau.cube.dataprovider.DataProvider");
            }
        }
    }
