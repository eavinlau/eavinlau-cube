package com.eavinlau.cube.solr;
...
import com.eavinlau.cube.dataprovider.util.DPCommonUtils;
import com.eavinlau.cube.exception.AJCubeException;
...
public class SolrDataProvider extends DataProvider implements Aggregatable, Initializing {
    ...
    public String[][] getSolrData() throws Exception {
        ...
        if (StringUtils.isBlank(solrServers))
            throw new AJCubeException("Datasource config Solr Servers can not be empty.");
        String collectionName = query.get("collection");
        if (StringUtils.isBlank(collectionName))
            throw new AJCubeException("Collection can not be empty.");
        ...
        if (results.size() > resultLimit) {
            throw new AJCubeException("Cube result count is greater than limit " + resultLimit);
        }
    ...
    public String[][] getSolrData(String dimColsStr, SolrDocumentList results) throws Exception {
        if (StringUtils.isEmpty(dimColsStr) || results == null || results.size() <= 0) {
            throw new AJCubeException("Cube result is null");
        }
